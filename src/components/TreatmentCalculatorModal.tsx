...
export const TreatmentCalculatorModal: React.FC<TreatmentCalculatorModalProps> = ({
  ...
  const generateWhatsAppMessage = () => {
    let msg = `*BOOKING APPOINTMENT & ESTIMASI - SOZO SKIN CLINIC*\n`;
    if (customerName) msg += `Nama Pasien: ${customerName}\n`;
    msg += `Cabang Outlet: ${selectedOutlet}\n`;
    msg += `Tipe Harga: ${useMemberPrice ? 'Member VIP' : 'Non-Member'}\n\n`;
    msg += `*Daftar Rangkaian Treatment:*\n`;
    ...
    cartItems.forEach((item, index) => {
      ...
      msg += `   _Isi: ${item.treatment.inclusions.join(', ')}_\n`;
    });

    msg += `\n-------------------------\n`;
    msg += `Subtotal: ${formatRupiah(subtotal)}\n`;
    msg += `Service Charge (5% max 150rb): ${formatRupiah(serviceCharge)}\n`;
    msg += `*Total Pembayaran: ${formatRupiah(totalWithService)}*\n`;
    msg += `DP Booking Appointment: ${formatRupiah(bookingDp)}\n`;
    msg += `*Sisa Pelunasan di Klinik: ${formatRupiah(remainingAtClinic)}*\n\n`;
    msg += `_Simulasi Cicilan 0% Paylater:_\n`;
    msg += `• 3x: ${formatRupiah(cicilan3x)}/bln\n`;
    msg += `• 6x: ${formatRupiah(cicilan6x)}/bln\n`;
    msg += `• 12x: ${formatRupiah(cicilan12x)}/bln\n\n`;
    msg += `Mohon konfirmasi ketersediaan jadwal dokter dan slot appointment. Terima kasih!`;
      ...
      <div className="bg-white rounded-3xl w-full max-w-2xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden border border-stone-200">
        ...
        <div className="p-4 bg-stone-50 border-t border-stone-200 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            <a
              href={cartItems.length > 0 ? `https://wa.me/${settings.csWhatsappNumber}?text=${generateWhatsAppMessage()}` : '#'}
              target="_blank"
