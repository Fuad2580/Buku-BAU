...
import { Tag, MapPin, Sparkles, Percent, MessageCircle } from 'lucide-react';
              ...
              {filteredPromos.map((item) => {
                ...
                return (
                  <tr key={item.id} className="hover:bg-rose-50/40 transition">
                    <td className="p-3.5 pl-5 font-bold text-stone-900">
                      {item.name}
                    </td>
                    <td className="p-3.5">
                    ...
                    <td className="p-3.5 pr-5 text-right">
                      <a
                        href={`https://wa.me/${settings.csWhatsappNumber}?text=${encodeURIComponent(
                          `Halo SOZO Skin Clinic, saya tertarik promo single: *${item.name}* (Harga: Rp ${item.memberPrice}.000). Mohon info jadwal dan reservasi.`
                        )}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-[#6B1D2F] hover:bg-[#521523] text-white text-[11px] font-bold shadow-xs transition"
                      >
                        <MessageCircle className="w-3 h-3 text-[#E8BF87]" />
                        <span>Tanya CS</span>
                      </a>
                    </td>
                  </tr>
